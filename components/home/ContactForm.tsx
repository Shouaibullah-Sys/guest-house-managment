...
export default function ContactForm({ 
    ...
    newFiles.forEach(file => {
      const interval = setInterval(() => {
        setUploadedFiles(prev => prev.map(f => 
          f.id === file.id ? { ...f, progress: Math.min(f.progress + 25, 100) } : f
        ));
  ...
  const removeFile = (id: string) => {
    ...
    setFormData(prev => ({
      ...prev,
      attachments: prev.attachments?.filter((_, index) => 
        uploadedFiles.findIndex(f => f.id === id) !== index
      )
    }));
  };
        ...
        {hotelInfo && (
          <div className="bg-amber-50 dark:bg-amber-900/20 rounded-xl p-4 mb-6 max-w-md mx-auto">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-medium text-amber-800 dark:text-amber-200">{hotelInfo.name}</span>
            </div>
            <p className="text-sm text-gray-600 dark:text-gray-300">
              We'll contact you regarding your inquiry about our property in {hotelInfo.location}
            </p>
          </div>
        )}
    ...
    >
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-4">
          {variant === 'luxury' && <Sparkles className="w-6 h-6 text-amber-500" />}
        ...
        {hotelInfo && (
          <div className="mt-4 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="font-semibold text-gray-900 dark:text-white">{hotelInfo.name}</h4>
                <p className="text-sm text-gray-600 dark:text-gray-300">{hotelInfo.location}</p>
              </div>
            </div>
          </div>
        )}
      ...
      <form onSubmit={onSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {formData.type === 'booking' && (
            <div className="space-y-2">
              ...
              {errors.guests && <p className={classes.error}>{errors.guests}</p>}
            </div>
          )}
        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      required
                    />
                    {errors.checkIn && <p className={classes.error}>{errors.checkIn}</p>}
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      required
                    />
                    {errors.checkOut && <p className={classes.error}>{errors.checkOut}</p>}
                  </div>
                </div>
        ...
        {enableNewsletter && (
          <div className="flex items-start gap-3 p-4 bg-amber-50 dark:bg-amber-900/20 rounded-lg">
            ...
            <label htmlFor="newsletter" className="text-sm text-gray-700 dark:text-gray-300">
              Subscribe to our newsletter for exclusive offers, updates, and luxury travel insights.
        ...
        <div className="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
          ...
          <p className="text-xs text-gray-600 dark:text-gray-400">
            Your information is protected with 256-bit SSL encryption. We respect your privacy and will never share your data with third parties.
          </p>
        </div>
        ...
        {variant === 'luxury' && (
          <div className="grid grid-cols-3 gap-4 pt-6 border-t border-amber-200/50 dark:border-amber-800/30">
            <div className="text-center">
              ...
              <p className="text-xs text-gray-600 dark:text-gray-400">Secure Booking</p>
            </div>
            <div className="text-center">
              <div className="w-10 h-10 bg-amber-100 dark:bg-amber-900/30 rounded-full flex items-center justify-center mx-auto mb-2">
                <CreditCard className="w-5 h-5 text-amber-600" />
              </div>
              <p className="text-xs text-gray-600 dark:text-gray-400">Best Price Guarantee</p>
            </div>
            <div className="text-center">
              <div className="w-10 h-10 bg-amber-100 dark:bg-amber-900/30 rounded-full flex items-center justify-center mx-auto mb-2">
