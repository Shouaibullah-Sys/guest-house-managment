...
function convertToDari(text: string): string {
  ...
  if (text.includes('$') || text.includes('افغانی')) {
    ...
    if (number) {
      const formatted = new Intl.NumberFormat('fa-AF').format(parseFloat(number.replace(/,/g, '')));
...
async function generateGuestReportExcel(guestsWithBookings: any[], reportDate: string, stats: any) {
...
function addSummarySheet(sheet: ExcelJS.Worksheet, reportDate: string, stats: any) {
  const HOTEL_NAME = "Nawaz Totakhail Hotel";
  ...
  titleCell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FF0052A5" } };
  titleCell.alignment = { horizontal: "center", vertical: "middle", wrapText: true };
  sheet.getRow(2).height = 30;

  // Report information in Dari
  sheet.getCell("A5").value = "معلومات راپور";
  sheet.getCell("A5").font = { size: 12, bold: true, color: { argb: "FF0052A5" }, name: 'Arial' };
  ...
  const reportData = [
    ...
    ["مجموع عواید:", `${new Intl.NumberFormat('fa-AF').format(stats.totalSpent)} افغانی`],
    ["متوسط خرج:", `${new Intl.NumberFormat('fa-AF').format(stats.averageSpent.toFixed(2))} افغانی`],
  ];
...
function addGuestsSheet(sheet: ExcelJS.Worksheet, guestsWithBookings: any[]) {
  const HOTEL_NAME = "Nawaz Totakhail Hotel";
  ...
  // Headers in Dari
  const headers = [
    "نام", "ایمیل", "شماره تلیفون", "تابعیت", "تعداد اقامت", 
    "تاریخ ورود", "تاریخ خروج", "آی دی کارت/پاسپورت", "شماره اطاق"
  ];
...
function addStatsSheet(sheet: ExcelJS.Worksheet, guestsWithBookings: any[], stats: any) {
  ...
  titleCell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FF27AE60" } };
  titleCell.alignment = { horizontal: "center", vertical: "middle" };
  ...
  const nationalityStats = guestsWithBookings.reduce((acc: Record<string, number>, guest: any) => {
  ...
  Object.entries(nationalityStats).forEach(([nationality, count], index) => {
    ...
    sheet.getCell(`B${index + 7}`).value = new Intl.NumberFormat('fa-AF').format(count as number);
  ...
  sheet.getCell(`A${summaryStartRow}`).font = { size: 12, bold: true, name: 'Arial' };

  const summaryData = [
    ["مجموع مهمانان:", new Intl.NumberFormat('fa-AF').format(stats.totalGuests)],
    ["مهمانان فعال:", new Intl.NumberFormat('fa-AF').format(stats.activeGuests)],
    ["مجموع عواید:", `${new Intl.NumberFormat('fa-AF').format(stats.totalSpent)} افغانی`],
    ["متوسط به هر مهمان:", `${new Intl.NumberFormat('fa-AF').format(stats.averageSpent.toFixed(2))} افغانی`],
    ["تعداد کل اقامت‌ها:", new Intl.NumberFormat('fa-AF').format(stats.totalStays)],
  ];
...
async function addLogoToSheet(sheet: ExcelJS.Worksheet, logoPath: string = '/logo.jpg') {
  try {
...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      const workbook = await generateGuestReportExcel(guestsWithBookings, reportDate, stats);
